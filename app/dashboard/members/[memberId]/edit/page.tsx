...
export default function EditMember({params} : {params:{memberId: string}}) {
  ...
  // const {data, trigger} = useSWRMutation(`/api/customers/${params.memberId}`, fetcher)
  const getMembers = async() => {
    const member = await fetch(`/api/customers/${params.memberId}`)
    const result = await member.json()
    console.log(result)
    setName(result.customers.name)
    setPhone(result.customers.phone)
    setEmail(result.customers.email)
  }
  ...
  const handleSubmit = async(e: any) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const dataCustomers = await fetch(`/api/customers/${params.memberId}`, {
          method: 'POST',
        ...
        const {success, customer} = await dataCustomers.json();
